...
We're primarily a PHP shop, but we are looking to gradually move away from PHP. PHP is still the preferred technology for this assignment, but we have components written in React.js, Node.js and Java, so feel free to mix these technologies in as well where they make sense.  Our minimal expectation is that there are some server side components written in a backend language.  The front end can be rendered on the client-side or server side, its entirely up to you.
